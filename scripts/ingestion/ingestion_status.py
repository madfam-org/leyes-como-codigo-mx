...
def print_dashboard(records: List[Dict], errors: List[Dict], timeframe: int = 24):
    ...
    success_rate = (
        sum(1 for r in records if r.get("grade") not in ["F", "N/A"])
        / len(records)
        * 100
    )
...
def main():
    ...
    parser.add_argument(
        "--last", type=int, default=24, help="Hours to look back (default: 24)"
    )
    parser.add_argument("--law", type=str, help="Show detail for specific law ID")
