...
def main():
    ...
    group.add_argument("--all", action="store_true", help="Ingest all laws")
    group.add_argument(
        "--priority", type=int, choices=[1, 2], help="Ingest laws by priority level"
    )
    group.add_argument(
        "--tier", type=str, help="Ingest laws by tier (e.g., fiscal, constitutional)"
    )
    group.add_argument(
    ...
    for law in laws:
        print(
            f"  • {law['id']:10} - {law.get('short_name', law.get('name', 'Unknown'))}"
        )
