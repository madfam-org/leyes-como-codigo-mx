...
def main():
    ...
    if args.laws:
        ...
        laws_to_validate = [
            l for l in registry["federal_laws"] if l.get("priority") == args.priority
        ]
    else:
    ...
    print("-" * 60)
    print(
        f"{'ID':<10} | {'Status':<10} | {'Actual':<8} | {'Expected':<8} | {'Diff %':<8}"
    )
    print("-" * 60)
    ...
    for law in laws_to_validate:
        ...
        if success:
            if isinstance(
                result, str
            ):  # Error in validation logic (shouldn't happen with success=True in my code structure above, wait)
            ...
            if isinstance(result, dict):
                print(
                    f"{result['id']:<10} | {result['status']:<10} | {result['actual']:<8} | {result['expected']:<8} | {result['percent']:.1f}%"
                )
