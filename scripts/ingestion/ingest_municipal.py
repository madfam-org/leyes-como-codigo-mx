...
def main():
    ...
    parser.add_argument(
        "--all", action="store_true", help="Process all implemented municipalities"
    )
    parser.add_argument("--muni", type=str, help="Specific municipality ID")
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Scrape catalog only, do not download files",
    )
    parser.add_argument(
        "--limit", type=int, default=0, help="Limit items per municipality"
    )
    ...
    if args.all:
        ...
        implemented = [
            m
            for m, status in list_available_scrapers().items()
