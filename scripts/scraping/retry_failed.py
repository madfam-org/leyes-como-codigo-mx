...
def main():
    ...
    for state_result in retry_states:
        ...
        if failed_count > 100:
                ...
                retry_stats["state_results"].append(
                    {
                        ...
                        "retry_success_rate": (
                            f"{recovered/failed_count*100:.1f}%"
                            if failed_count > 0
                            else "N/A"
                        ),
                    }
            ...
            log(
                f"   ⚠️  {state_name}: {failed_count} failures (manual review recommended)"
            )
            retry_stats["total_retried"] += failed_count
    ...
    log(
        f"Recovery rate: {retry_stats['recovered']/retry_stats['total_retried']*100:.1f}%"
    )
