...
def classify_law_category(law_name: str, text: str) -> str:
    ...
    for category, patterns in CATEGORY_PATTERNS.items():
        score = sum(
            1 for pattern in patterns if re.search(pattern, search_text, re.IGNORECASE)
        )
...
def process_law_file(txt_path: Path, original_metadata: Dict) -> Dict:
    ...
    try:
        ...
        official_id = (
            f"{state.lower().replace(' ', '_')}_{txt_path.stem.rsplit('_', 1)[0]}"
        )
        ...
        return {"success": True, "official_id": official_id, "metadata": enhanced}

    except Exception as e:
        return {"success": False, "txt_path": str(txt_path), "error": str(e)}
...
def main():
    ...
    group.add_argument("--all", action="store_true", help="Process all state laws")
    group.add_argument(
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="data/state_laws_metadata.json",
    ...
    if args.all:
        ...
        state_dir = Path("data/state_laws_processed") / args.state.lower().replace(
            " ", "_"
        )
