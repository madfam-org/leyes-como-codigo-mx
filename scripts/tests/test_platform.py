...
def test_schema_validation(xml_dir: Path) -> Dict:
    ...
    for xml_file in xml_files:
        ...
        results.append(
            {"law": law_name, "valid": result.is_valid, "errors": len(result.errors)}
        )
...
def test_completeness_validation(xml_dir: Path) -> Dict:
    ...
    for xml_file in xml_files:
        ...
        print(
            f"  {law_name:15s} {status} ({report.passed_checks}/{report.total_checks} checks, {report.error_count} issues)"
        )
    ...
    avg_pass = (
        sum(r["passed"] / r["total"] for r in results if r["total"] > 0)
        / len(results)
        * 100
    )
...
def test_quality_metrics(xml_dir: Path) -> Dict:
    ...
    for xml_file in xml_files:
        ...
        print(
            f"  {law_slug:15s} Grade {metrics.grade} ({metrics.overall_score:.1f}%) - {metrics.articles_found} articles"
        )
...
""".strip()
    ...
    print(
        f"  TRANSITORIOS found: {len([a for a in result.articles if 'trans-' in a['id']])}"
    )
    print(f"  Confidence: {result.metadata['confidence']*100:.1f}%")

    # Test FRBR metadata creation
    metadata = parser.create_frbr_metadata(
        law_type="ley", date_str="2024-01-01", slug="test", title="Test Law"
    )
    ...
    success = (
        result.metadata["articles"] >= 3
        and result.metadata["titles"] >= 1
        and result.metadata["confidence"] > 0.9
    )
...
def main():
    ...
    try:
        ...
        print(
            f"✅ Completeness Checks: {results['completeness']['average']:.1f}% average"
        )
        print(f"✅ Quality Metrics: {results['quality']['average']:.1f}% average")
        print(
            f"✅ Parser Functionality: {'PASS' if results['parser']['success'] else 'FAIL'}"
        )
        print(
            f"✅ CLI Tools: {results['cli']['passed']}/{results['cli']['total']} working"
        )

        # Overall assessment
        overall_success = (
            results["schema"]["valid"] == results["schema"]["total"]
            and results["completeness"]["average"] >= 80
            and results["quality"]["average"] >= 90
            and results["parser"]["success"]
            and results["cli"]["passed"] == results["cli"]["total"]
        )
