...
EXCLUDE_DIRS = {
    ...
    'htmlcov', '.pytest_cache', 'dist', 'build', '.next', 'engines'
}
...
def analyze_file(file_path: Path) -> dict:
    ...
    try:
        ...
        avg_methods_per_class = (
            sum(len(methods) for methods in analyzer.class_methods.values()) / len(analyzer.classes)
            if analyzer.classes else 0
        )
...
def print_analysis(candidates: dict, all_files: list):
    ...
    complex_files = sorted(all_files, 
    ...
    for f in complex_files:
        ...
        print(f"    └─ {f['lines']} lines, {f['functions']} funcs, {f['classes']} classes, {f['imports']} imports")
