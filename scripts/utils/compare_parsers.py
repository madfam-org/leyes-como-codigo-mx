...
def compare_parsers():
    ...
    for elem_type in ["books", "titles", "chapters", "sections", "articles", "notes"]:
        ...
        print(
            f"{icon} {elem_type.capitalize():<12} {v1_count:<10} {v2_count:<10} {change_str}"
        )
    ...
    if metrics["v2"]["titles"] > metrics["v1"]["titles"]:
        improvements.append(
            f"✅ TÍTULO detection: {metrics['v1']['titles']} → {metrics['v2']['titles']} ({metrics['v2']['titles']} found!)"
        )

    if metrics["v2"]["sections"] > metrics["v1"]["sections"]:
        improvements.append(
            f"✅ SECTION detection: +{metrics['v2']['sections']} sections parsed"
        )
    ...
    if transitorios_count > 0:
        improvements.append(
            f"✅ TRANSITORIOS parsing: {transitorios_count} transitory articles"
        )

    if metrics["v2"]["notes"] > 0:
        improvements.append(
            f"✅ Reform metadata: {metrics['v2']['notes']} annotations extracted"
        )

    article_diff = metrics["v2"]["articles"] - metrics["v1"]["articles"]
    if article_diff != 0:
        improvements.append(
            f"ℹ️  Article count: {metrics['v1']['articles']} → {metrics['v2']['articles']} ({article_diff:+d})"
        )
    ...
    print(
        f"V1 Accuracy: ~{v1_accuracy:.1f}% ({metrics['v1']['articles']}/{expected_articles} articles)"
    )
    print(
        f"V2 Accuracy: ~{v2_accuracy:.1f}% ({metrics['v2']['articles']}/{expected_articles} articles)"
    )
