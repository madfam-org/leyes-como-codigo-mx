...
from apps.parsers.patterns.metadata import compile_reform_pattern, extract_reforms
from apps.parsers.patterns.structure import compile_structure_patterns, roman_to_int
...
class TestMetadataPatterns:
    ...
    def test_extract_reforms(self):
        ...
        assert (
            "reformada" in reforms[0]["action"].lower()
            or "adicionado" in reforms[0]["action"].lower()
        )
