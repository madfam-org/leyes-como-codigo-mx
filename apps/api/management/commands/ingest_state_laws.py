...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        group.add_argument('--all', action='store_true',
                          help='Ingest all state laws')
        group.add_argument('--state', type=str,
                          help='Ingest specific state')
    ...
    def create_law_and_version(self, metadata, dry_run=False):
        ...
        try:
            ...
            text_content = Path('/app/' + text_file).read_text(encoding='utf-8', errors='ignore')
    ...
    def handle(self, *args, **options):
        ...
        if not metadata_file.exists():
            self.stdout.write(self.style.ERROR(f"Metadata file not found: {metadata_file}"))
        ...
        for i in range(0, len(all_laws), options['batch_size']):
            ...
            if not options['dry_run']:
                with transaction.atomic():
                    for law_metadata in batch:
                        result = self.create_law_and_version(law_metadata, options['dry_run'])
            ...
            self.stdout.write(f"  Batch {batch_count}: {processed:,}/{len(all_laws):,} "
