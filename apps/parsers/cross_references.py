...
class CrossReferenceDetector:
    ...
    PATTERNS = [
        ...
        (
            r"art[íi]culo\s+(\d+[A-Z]?(?:-[A-Z])?)\s+de\s+(?:la|el)\s+([Ll]ey\s+[^,\.\;]+|C[óo]digo\s+[^,\.\;]+|Reglamento\s+[^,\.\;]+)",
            2.0,
        ),
        # "[Law Name], artículo 5"
        (
            r"([Ll]ey\s+[^,\.]{5,}|C[óo]digo\s+[^,\.]{5,}|Reglamento\s+[^,\.]{5,}),\s+art[íi]culo\s+(\d+[A-Z]?(?:-[A-Z])?)",
            2.0,
        ),
        # "fracción III del artículo 107"
        (
            r"fracci[óo]n\s+([IVXLCDM]+)\s+del\s+art[íi]culo\s+(\d+[A-Z]?(?:-[A-Z])?)",
            1.8,
        ),
        ...
        (
            r"((?:[Ll]ey|C[óo]digo|Reglamento)\s+(?:Federal|General|Estatal|Municipal|Org[áa]nic[oa])\s+[^,\.\;]{10,})",
            1.2,
        ),
    ]
    ...
    def _extract_reference(
        self, match: re.Match, text: str, pattern_weight: float
    ...
    def resolve_law_slug(
        self, law_name: str, law_slugs: Dict[str, str]
    ) -> Optional[str]:
        """
