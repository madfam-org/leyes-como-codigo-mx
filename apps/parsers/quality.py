...
from apps.parsers.validators import (
    AKNSchemaValidator,
    CompletenessReport,
...
class QualityMetrics:
    ...
    def total_issues(self) -> int:
        ...
        return len(self.schema_errors) + sum(
            len(issues) for issues in self.completeness_issues.values()
        )
...
class QualityCalculator:
    ...
    def calculate(
        ...
        try:
            ...
            metrics.transitorios = len(
                [a for a in articles if "trans-" in a.get("id", "")]
            )
        ...
        metrics.accuracy_score = self._calculate_accuracy(
            metrics.articles_found, articles_expected, parser_confidence
        )

        metrics.completeness_score = self._calculate_completeness(
            metrics, completeness_result
        )

        return metrics

    def _calculate_accuracy(
        self, found: int, expected: int, confidence: float
    ) -> float:
        """
    ...
    def _calculate_completeness(
        self, metrics: QualityMetrics, completeness_result: CompletenessReport
    ) -> float:
        """
    ...
    def print_report(self, metrics: QualityMetrics):
        ...
        print(
            f"  Speed:        {metrics.file_size_mb/metrics.parse_time_seconds if metrics.parse_time_seconds > 0 else 0:.2f} MB/s"
        )
