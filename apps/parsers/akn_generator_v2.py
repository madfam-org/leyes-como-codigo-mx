...
class AkomaNtosoGeneratorV2:
    ...
    def create_frbr_metadata(
        self, law_type: str, date_str: str, slug: str, title: str
    ) -> Dict[str, Any]:
    ...
    def _try_patterns(
        self, text: str, patterns: List, line_num: int = 0
    ) -> Tuple[bool, Any]:
        """
    ...
    def _find_structure_elements(
        self, lines: List[str], element_type: str
    ) -> List[Dict]:
        """
        ...
        for i, line in enumerate(lines):
            ...
            if matched:
                ...
                if i + 1 < len(lines):
                    ...
                    is_structure = any(
                        self._try_patterns(next_line, self.structure_patterns[k])[0]
    ...
    def _article_confidence(self, content: str) -> float:
        ...
        if re.search(
            r"(en términos|conforme|de acuerdo|lo dispuesto)", content, re.IGNORECASE
        ):
            score += 0.05
    ...
    def parse_structure_v2(self, text: str) -> ParseResult:
        ...
        for struct_type in ["book", "title", "part", "chapter", "section"]:
            ...
            if struct_type in ["book", "title"] and len(elements) == 0:
                result.add_warning(
                    f"No {struct_type.upper()} elements found - verify structure"
                )
        ...
        if len(transitorios) == 0:
            result.add_warning(
                "No TRANSITORIOS found - verify if law has transitory articles"
            )
        ...
        result.metadata = {
            ...
            "structure": {
                k: len([e for e in result.elements if e["type"] == k])
    ...
    def generate_xml(
        self, text: str, metadata: Dict[str, Any], output_path: Path
        ...
        etree.SubElement(
            expression, "FRBRuri", value=metadata["expression_uri"].rsplit("/", 1)[0]
        )
        etree.SubElement(
            expression, "FRBRdate", date=metadata["date"], name="Generation"
        )
        ...
        etree.SubElement(
            manifestation,
            "FRBRuri",
            value=metadata["manifestation_uri"].replace(".xml", ".akn"),
        )
        etree.SubElement(
            manifestation, "FRBRdate", date=str(date.today()), name="Generation"
        )
        ...
        for elem in elements:
            ...
            if elem_type == "book":
                ...
                para = etree.SubElement(
                    article_elem, "paragraph", id=f"{elem['id']}-para-1"
                )
