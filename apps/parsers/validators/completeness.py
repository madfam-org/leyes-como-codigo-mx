...
class CompletenessReport:
    ...
    def summary(self) -> str:
        ...
        return (
            f"⚠️  {self.total_issues} issue(s) found across {len(self.issues)} check(s)"
        )
...
class CompletenessValidator:
    ...
    def validate(self, xml_path: Union[Path, str]) -> CompletenessReport:
        ...
        report = CompletenessReport(file_path=xml_path, timestamp=datetime.now())
    ...
    def check_article_sequence(self, root: etree.Element, xml_path: Path) -> List[str]:
        ...
        for num, text in article_numbers:
            if prev_num is not None:
                ...
                if gap > 5:
                    issues.append(
                        f"Large gap: Article {prev_num} → {num} (gap of {gap})"
                    )
            prev_num = num
    ...
    def check_content_length(self, root: etree.Element, xml_path: Path) -> List[str]:
        ...
        for article in articles:
            ...
            total_content = "".join(elem.text or "" for elem in content_elems)
    ...
    def check_hierarchy(self, root: etree.Element, xml_path: Path) -> List[str]:
        ...
        if structure_counts["chapter"] > 0:
            articles_per_chapter = (
                structure_counts["article"] / structure_counts["chapter"]
            )
    ...
    def check_structure_elements(
        self, root: etree.Element, xml_path: Path
    ) -> List[str]:
