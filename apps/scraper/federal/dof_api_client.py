...
class DOFAPIClient:
    ...
    def download_law_from_diputados(
        self, law_slug: str, save_path: Optional[Path] = None
    ) -> Optional[Path]:
        """
...
def main():
    ...
    dof_pdf = client.get_daily_pdf(
        date=publication_date, save_path=Path("data/raw/dof_2013-04-02.pdf")
    )
    ...
    chamber_pdf = client.download_law_from_diputados(
        law_slug="Ley_de_Amparo", save_path=Path("data/raw/ley_de_amparo.pdf")
    )
