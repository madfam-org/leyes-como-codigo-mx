...
class CongressCrawler:
    ...
    def fetch_page(self, url: str) -> Optional[str]:
        try:
            response = self.session.get(url, timeout=20, verify=False) # Skip verify for some gov sites
    ...
    def find_municipal_links(self) -> List[Dict]:
        ...
        for a in soup.find_all('a', href=True):
            ...
            is_relevant = (
                ('municip' in text or 'municip' in href_lower) and
                ('ingreso' in text or 'bando' in text or 'hacienda' in text or 'ley' in text) and
                ('.pdf' in href_lower)
            )
