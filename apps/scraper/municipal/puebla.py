...
Scrapes regulations from Puebla's municipal transparency portal.
"""
...
class PueblaScraper(MunicipalScraper):
    ...
    def scrape_catalog(self) -> List[Dict]:
        """
        Scrape Puebla's regulations catalog.

        Returns list of law dictionaries.
        """
        # Try transparency/normatividad path from config
        catalog_path = self.selectors.get("catalog_path", "/transparencia/normatividad")
        ...
        if not html:
            ...
            alternatives = ["/normatividad", "/transparencia", "/reglamentos"]
